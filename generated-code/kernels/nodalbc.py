...
def addKernels(
    ...
    easi_ident_map = np.stack(
        [np.eye(aderdg.numberOfQuantities())]
    ...
    generator.add(
        "createEasiBoundaryGhostCells", create_easi_boundary_ghost_cells
    )

    projectToNodalBoundary = (
        lambda j: aderdg.INodal["kp"]
        <= aderdg.db.V3mTo2nFace[j]["km"] * aderdg.I["mp"]
    )
    ...
    updateINodal = (
        aderdg.INodal["kp"]
        <= aderdg.INodal["kp"] + factor * aderdg.INodalUpdate["kp"]
    )
