...
format() {
    ...
    if [ "${formatter_version}" != "clang-format version 23.1.0" ]; then
        echo "Your clang-format tool in \"${formatter}\" does not have the correct version (should be 23.1.0). Given: ${formatter_version}"
        echo "Hint: you may install the required clang-format via pip, by typing: pip3 install clang-format==23.1.0"
        exit 176
    fi
