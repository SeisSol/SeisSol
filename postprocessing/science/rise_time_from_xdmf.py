...
parser.add_argument('--threshold', nargs=1, metavar=('threshold'), default=([0.1]), help='specify slip rate threshold' ,type=float)
parser.add_argument('--events', nargs=1, metavar=('events'), choices=[1,2], default=([1]), help='number of events [1,2] in xdmf file' ,type=int)
parser.add_argument('--median_filter_radius', nargs=1, default=([0.]), metavar=('radius of the median filter in m'),
...
parser.add_argument('--nprocs', nargs=1, metavar=('number of processes'), default=([4]),
                    help='use multiprocessing to speed up the median filtering' ,type=int)
parser.add_argument('--complicated_fault',  dest='complicated_fault', action='store_true' , default = False, help='Improves median filter for branching faults and removes isolated slipping elements')
...
def MedianSmoothing(input_tuple):
    ...
    for i, j in enumerate(indices):
        ...
        if complicated_fault:
            ...
            if non_zero_portion < 0.75 and variable[j] == 0:       # improve filter for branching faults
                variableSmooth[i] = 0
...
if args.median_filter_radius[0]:
    ...
    for i in range(args.events[0]):
        RT[i] = MedianSmoothingParallel(RT[i], xyz, radius=args.median_filter_radius[0], nprocs=args.nprocs[0])
