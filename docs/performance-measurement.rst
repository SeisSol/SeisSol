...
Kernel Bandwidth (B/s)
----------------------

SeisSol outputs one bandwidth estimation number, namely the kernel bandwidth.
...
Due to the structure of SeisSol, the kernel bandwidth will work better for GPUs
than CPUs: on the GPUs (and not on CPUs), a loop over all elements corresponds to a single kernel.
As such, the kernel bandwidth should be somewhere between L2 and main memory bandwidth
for most kernels.
