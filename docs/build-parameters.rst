...
- ``MEMORY_LAYOUT``: the sparsity patterns to apply. If set to ``auto``, it is chosen from the
  target-specific config folder (``codegen/config/cpu`` or ``codegen/config/gpu``). If you pass
  a filename such as ``dense.xml`` or ``ms32.xml``, it is resolved from that same target-specific
