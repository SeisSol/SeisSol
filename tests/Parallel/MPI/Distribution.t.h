...
TEST_CASE("Distributor cross-rank exchange" * doctest::test_suite("mpi")) {
  ...
  for (std::size_t i = 0; i < ElemsPerRank; ++i) {
    const double expected = static_cast<double>(targetIds[i]) * 7.0 + 3.0;
...
TEST_CASE("Distributor single element" * doctest::test_suite("mpi")) {
  ...
  const std::vector<std::size_t> sourceIds = {static_cast<std::size_t>(rank)};

  // Each rank requests from the next rank (circular)
  const std::vector<std::size_t> targetIds = {static_cast<std::size_t>((rank + 1) % size)};

  const int sourceData = rank * 1000;
  ...
  const int expectedRank = (rank + 1) % size;
