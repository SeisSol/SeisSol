...
#pragma omp parallel for schedule(static) private(l_timeIntegrated, l_faceNeighbors_prefetch)
      ...
      for( unsigned int l_cell = 0; l_cell < layer.getNumberOfCells(); l_cell++ ) {
...
#ifdef _OPENMP
                                                       *reinterpret_cast<real (*)[4][tensor::I::size()]>(&(globalData.onHost->integrationBufferLTS[omp_get_thread_num()*4*tensor::I::size()])),
#else
        ...
        neighborKernel.computeNeighborsIntegral( data,
                                                   drMapping[l_cell],
