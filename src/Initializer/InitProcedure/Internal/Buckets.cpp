...
class BucketManager {
  ...
  public:
  template <typename T>
  std::remove_pointer_t<T>* markAllocate(bool allocate) {
    if (allocate) {
...
std::vector<CommunicationInfo>
  ...
  auto allocate = [&](std::size_t index, bool useDerivatives) {
    ...
    if (useDerivatives) {
      derivatives[index] =
          manager.markAllocate<typename decltype(lts.derivatives)::Type>(hasDerivatives);
    } else {
      buffers[index] = manager.markAllocate<typename decltype(lts.buffers)::Type>(hasBuffers);
    }
  };
  ...
  for (unsigned cell = 0; cell < layer.size(); ++cell) {
    for (int face = 0; face < 4; ++face) {
      faceDisplacements[cell][face] =
          manager
              .markAllocate<std::remove_extent_t<typename decltype(lts.faceDisplacements)::Type>>(
                  seissol::initializer::requiresDisplacement(
...
void bucketsAndCommunication(memory::MemoryContainer& container,
  ...
  for (auto& layer : container.volume.leaves()) {
    ...
    if (layer.getIdentifier().halo == HaloType::Ghost) {
      commInfo[layer.id()] =
          allocateTransferInfo(layer, container.wpdesc, clusterLayout.ghostRegions);
    }
    if (layer.getIdentifier().halo == HaloType::Copy) {
      // TODO: for all regions, setup
      commInfo[layer.id()] =
          allocateTransferInfo(layer, container.wpdesc, clusterLayout.copyRegions);
    }
