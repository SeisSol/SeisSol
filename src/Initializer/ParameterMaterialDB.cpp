...
namespace {
    struct ModelQuadratureRule {
        ...
        ::std::vector<::std::array<double, 3>> map(const ::std::array<Eigen::Vector3d, 4>& vertices) const {
            ...
            for (size_t i = 0; i < points.size(); ++i) {
                ::seissol::transformations::tetrahedronReferenceToGlobal(verticesConv[0].data(), verticesConv[1].data(), verticesConv[2].data(), verticesConv[3].data(), points[i].data(), mappedPoints[i].data());
            }
        }
    };
    ...
    public:
        ...
        const ModelQuadratureRule& getRule(int order) const {
            return rules[order];
        }
    private:
    ...
    struct MaterialAverager {
        ...
        static MaterialT compute(const ::std::vector<MaterialT>& materials, size_t start, const ModelQuadratureRule& rule) {
    ...
    // TODO(David):: move both MaterialBinder and MaterialPostprocessor to the material classes themselves, maybe make inheritable
    ...
    struct MaterialPostprocess<::seissol::model::ViscoElasticMaterial<Mechanisms>> {
        static void postprocess(::seissol::model::ViscoElasticMaterial<Mechanisms>& material)
        {
            ...
            ::seissol::physics::fitAttenuation(material, parameters.model.freqCentral, parameters.model.freqRatio);
        }
    };
    ...
    template<typename QueryMaterialT, typename OutputMaterialT>
    auto convertMaterial(QueryMaterialT&& source) -> decltype(OutputMaterialT(::std::forward<QueryMaterialT>(source)))
    {
        return ::std::move(OutputMaterialT(::std::forward<QueryMaterialT>(source)));
    }

    // TODO(David): add up/downcasts here (partially done). E.g. Elastic <=> Acoustic, Elastic <=> Anisotropic etc.

    /*template<> // NOT YET!
    model::PlasticityData<> convertMaterial<model::Plasticity, model::PlasticityData<>>(model::Plasticity&& material)
    {
    ...
    void runMaterialQuery(::std::vector<OutputArrayT>& output, const ::std::vector<MaterialInfo>& materialInfo, const ::std::string& modelFile, const initializers::CellToVertexArray& ctov, const ModelQuadratureRuleCollection& rules, bool postprocess) {
        if constexpr (::std::is_same_v<QueryMaterialT, OutputMaterialT>) {
            logInfo() << "Material query for" << QueryMaterialT::Text << "(" << materialInfo.size() << "cells)";
        }
        else {
            logInfo() << "Material query for" << QueryMaterialT::Text << "=>" << OutputMaterialT::Text << "(" << materialInfo.size() << "cells)";
        }
        ...
        for (size_t i = 0; i < materialInfo.size(); ++i) {
            auto mapped = rules.getRule(materialInfo[i].order).map(ctov.elementCoordinates(materialInfo[i].index));
        ...
        for (size_t i = 0; i < count; ++i) {
            auto averagedMaterial = materialInfo[i].order == 0 ? tempOutput[materialInfo[i].uncompressedIndex]
                : MaterialAverager<QueryMaterialT>::compute(tempOutput, materialInfo[i].uncompressedIndex, rules.getRule(materialInfo[i].order));
            auto outputMaterial = ::std::move(convertMaterial<QueryMaterialT, OutputMaterialT>(::std::move(averagedMaterial)));
            if (postprocess) {
...
namespace seissol::model {
    ...
    ::std::pair<::std::vector<SupportedMaterials>, ::std::vector<::seissol::model::Plasticity>> queryMaterial(const initializer::CellConfigInfoMap& infoMap, const initializers::CellToVertexArray& ctov, bool fullCompute) {
        ...
        for (const auto& [groupArg,info] : infoMap) {
            ...
            // (for now, no compression; i.e. different groups with the same material get computed in two passes)
        ...
        for (::std::size_t i = 0; i < compressedInfoMap.size(); ++i) {
            ...
            if (!materials.empty()) {
                ...
                ::std::visit([&](const auto& config) {
                    ...
                    // TODO(David): add functionality to use a different easi file material than is outputted (by casting up/down)
                    runMaterialQuery<MaterialT, MaterialT>(materialOutput, materials, exampleInfo.model, ctov, rules, fullCompute);
                    if (fullCompute) {
                        runMaterialQuery<Plasticity, Plasticity>(plasticityOutput, plasticities, exampleInfo.model, ctov, rules, fullCompute);
                    }
                }, exampleInfo.config);
            }
        }
