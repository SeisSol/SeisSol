...
void invertAgainst(real* matC, const LeftT& matA, const yateto::DenseTensorView<N, RealT, IntT>& matB) {
    if constexpr (N == 3) {
        for (int i = 0; i < matB.shape(2); ++i) {
            invertAgainst<ResT>(matC, matA, matB.subtensor(yateto::slice<>(), yateto::slice<>(), i));
            matC += ResT::size(i);
        }
    }
    else {
...
void GlobalMatrixPointers::bootstrapMatrices() {
    ...
    auto matk = seissol::init::ew_M::view::create(datak);
    auto matkT = seissol::init::ew_M::view::create(datakT);
    auto matr = seissol::init::ew_M::view::create(datar);
