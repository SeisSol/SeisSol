...
namespace seissol {
    namespace initializers{
        ...
        struct LTSRef {
            ...
            LTSRef(std::size_t id, LTSTree& tree) : id(id), tree(tree), lts(dynamic_cast<const LtsT&>(tree.lts())) {}
        ...
        struct LTSLayerRef {
            ...
            LTSLayerRef(std::size_t config, std::size_t cluster, LayerType icg, LTSTree& tree, Layer& layer)
                : config(config), cluster(cluster), icg(icg), tree(tree), layer(layer), lts(dynamic_cast<const LtsT&>(tree.lts())) {}
        ...
        public:
            ...
            constexpr void visitTwoLayers(F&& visitor, LTSForest<T, Lts2T>& two) {
                visitTwoInternalLayers<0, F, Lts2T>(std::forward<F>(visitor), two);
            }
            ...
            constexpr void visitLayersInternal(F&& visitor) {
                if constexpr(Idx < std::variant_size_v<T>) {
                    ...
                    for (std::size_t i = 0; i < children[Idx].numChildren(); ++i) {
                        ...
                        visitor(std::forward<LTSLayerRef<LTSType>>(LTSLayerRef<LTSType>(Idx, i, Ghost, children[Idx], child.template child<Ghost>())));
                        visitor(std::forward<LTSLayerRef<LTSType>>(LTSLayerRef<LTSType>(Idx, i, Copy, children[Idx], child.template child<Copy>())));
                        visitor(std::forward<LTSLayerRef<LTSType>>(LTSLayerRef<LTSType>(Idx, i, Interior, children[Idx], child.template child<Interior>())));
                    }
            ...
            constexpr void visitTwoLayersInternal(F&& visitor, LTSForest<T, Lts2T>& two) {
                if constexpr(Idx < std::variant_size_v<T>) {
                    ...
                    for (std::size_t i = 0; i < children[Idx].numChildren(); ++i) {
                        ...
                        visitor(LTSLayerRef<LTSType>(Idx, i, Interior, children[Idx], child.template child<Interior>(),
                            LTSLayerRef<LTS2Type>(Idx, i, Interior, children[Idx], child.template child<Interior>())));
                    }
