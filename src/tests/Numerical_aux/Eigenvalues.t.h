...
void testResidual(std::array<std::complex<double>,dim*dim>& M, seissol::eigenvalues::Eigenpair<std::complex<double>, dim>& eigenpair) {
  ...
  for (unsigned i = 0; i < dim*dim; i++) {
    REQUIRE(std::abs(M_R[i] - R_L[i]) == doctest::Approx(0.0).epsilon(epsilon));
  }
}
