...
TEST_CASE("Gaussian Nucleation Function") {
  ...
  for (real const& effectiveRiseTime : {0.8, 1.0}) {
      for (int i = -10; i < 111; i++) {
        real stfEvaluated = seissol::gaussianNucleationFunction::smoothStepIncrement(i * dt, dt, effectiveRiseTime);
        real referenceEvaluated = gaussianNucleation(i * dt, dt, effectiveRiseTime);
