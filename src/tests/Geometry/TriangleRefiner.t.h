...
void assertTriangle(seissol::refinement::Triangle& a,
                    ...
                    double area,
                    double epsilon) {
  REQUIRE(a.area == doctest::Approx(area).epsilon(epsilon));
  for (int i = 0; i < 3; i++) {
    REQUIRE(a.x[i][0] == doctest::Approx(b[i][0]).epsilon(epsilon));
    REQUIRE(a.x[i][1] == doctest::Approx(b[i][1]).epsilon(epsilon));
  }
}
