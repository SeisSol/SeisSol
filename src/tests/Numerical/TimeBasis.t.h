  ...
  for (std::size_t i = 1; i < 10; ++i) {
    ...
    for (const auto& point : {0.0, 1.0, 2.0, 0.123, 15.0}) {
      ...
      for (std::size_t j = 0; j < i; ++j) {
        ...
        REQUIRE(expD[j] == AbsApprox(static_cast<RealT>(functions::shiftedLegendre(j, tau, 1))));
        REQUIRE(expI[j] == AbsApprox(static_cast<RealT>(functions::shiftedLegendre(j, tau, -1) -
