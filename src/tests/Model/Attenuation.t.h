...
TEST_CASE("Attenuation") {
    ...
    // The test data used here was picked from one cell in a coarse discretization of the Northridge scenario.
    ...
    if (NUMBER_OF_RELAXATION_MECHANISMS == 3) {
        ...
        INFO("The attenuation test has been skipped, since the test data is tuned to 3 mechanisms only.");
    }
}
} // namespace seissol::unit_test
