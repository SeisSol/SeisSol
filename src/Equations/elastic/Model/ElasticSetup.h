...
inline void getTransposedGodunovStateElastic(const ElasticMaterial& local,
  ...
  if (faceType == FaceType::FreeSurface) {
    ...
    getTransposedFreeSurfaceGodunovState<ElasticMaterial>(materialtype, QgodLocal, QgodNeighbor, R);
  } else {
...
static void getFaceRotationMatrixElastic(const VrtxCoords normal,
...
template <typename Cfg>
struct MaterialSetup<Cfg, std::enable_if_t<Cfg::MaterialType == MaterialType::Elastic>> {
  ...
  static void getTransposedGodunovState(const ElasticMaterial& local,
    ...
    if (faceType == FaceType::FreeSurface) {
      const MaterialType materialtype =
          testIfAcoustic(local.mu) ? MaterialType::Acoustic : MaterialType::Elastic;
      getTransposedFreeSurfaceGodunovState<MaterialT>(materialtype, qGodLocal, qGodNeighbor, matR);
    } else {
      Matrix99 chi = Matrix99::Zero();
      if (!testIfAcoustic(local.mu)) {
        chi(2, 2) = 1.0;
  ...
  static void initializeSpecificNeighborData(const ElasticMaterial& material, void* localData) {}
