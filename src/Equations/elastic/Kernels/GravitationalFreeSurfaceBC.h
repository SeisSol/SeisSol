...
class GravitationalFreeSurfaceBc {
  ...
  static std::pair<long long, long long>
    ...
    constexpr auto numberOfNodes = Yateto<Config>::Tensor::nodal::nodes2D::Shape[0];
    ...
    for (int order = 1; order < Config::ConvergenceOrder + 1; ++order) {
#ifdef USE_ELASTIC
      ...
      nonZeroFlops += Yateto<Config>::Kernel::projectDerivativeToNodalBoundaryRotated::nonZeroFlops(
                          order - 1, face) +
                      flopsUpdates;
      hardwareFlops +=
          Yateto<Config>::Kernel::projectDerivativeToNodalBoundaryRotated::hardwareFlops(order - 1,
                                                                                         face) +
          flopsUpdates;
    }

    // Two rotations: One to face-aligned, one to global
    hardwareFlops += 2 * Yateto<Config>::Kernel::rotateFaceDisplacement::HardwareFlops;
    nonZeroFlops += 2 * Yateto<Config>::Kernel::rotateFaceDisplacement::NonZeroFlops;
  ...
  void evaluate(unsigned faceIdx,
    ...
    auto T = Yateto<Config>::Init::Tinv::view::create(boundaryMapping.TData);
    ...
    alignas(Alignment) RealT
        rotateDisplacementToFaceNormalData[Yateto<Config>::Init::displacementRotationMatrix::Size];
    ...
    alignas(Alignment) RealT
        rotateDisplacementToGlobalData[Yateto<Config>::Init::displacementRotationMatrix::Size];
    auto rotateDisplacementToGlobal =
        Yateto<Config>::Init::displacementRotationMatrix::view::create(
            rotateDisplacementToGlobalData);
    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < 3; ++j) {
    ...
    static_assert(Yateto<Config>::Init::rotatedFaceDisplacement::Size ==
                  Yateto<Config>::Init::faceDisplacement::Size);
    alignas(Alignment)
        RealT rotatedFaceDisplacementData[Yateto<Config>::Init::rotatedFaceDisplacement::Size];

    auto integratedDisplacementNodal =
        Yateto<Config>::Init::averageNormalDisplacement::view::create(
            integratedDisplacementNodalData);
    auto rotatedFaceDisplacement =
        Yateto<Config>::Init::faceDisplacement::view::create(rotatedFaceDisplacementData);

    // Rotate face displacement to face-normal coordinate system in which the computation is
    // more convenient.
    auto rotateFaceDisplacementKrnl = typename Yateto<Config>::Kernel::rotateFaceDisplacement();
    ...
    alignas(Alignment) RealT dofsFaceNodalStorage[Yateto<Config>::Tensor::INodal::size()];
    auto dofsFaceNodal = Yateto<Config>::Init::INodal::view::create(dofsFaceNodalStorage);

    // Temporary buffer to store nodal face coefficients at some time t
    alignas(Alignment) std::array<RealT, Yateto<Config>::Tensor::nodal::nodes2D::Shape[0]>
        prevCoefficients;
    ...
    const double Z = MaterialT::Type == seissol::model::MaterialType::elastic
                         ? std::sqrt(materialData.lambda * rho)
                         : 1;                        // TODO non-elastic
    const double g = getGravitationalAcceleration(); // [m/s^2]

    // Note: Probably need to increase ConvergenceOrder 1 here!
    for (int order = 1; order < Config::ConvergenceOrder; ++order) {
...
#pragma omp simd
      for (unsigned int i = 0; i < Yateto<Config>::Tensor::nodal::nodes2D::Shape[0]; ++i) {
        ...
        const double curCoeff =
            MaterialT::Type == seissol::model::MaterialType::elastic
    ...
    if (dataTable.find(key) != dataTable.end()) {
      ...
      std::array<std::tuple<RealT***, RealT**, unsigned long>, 5> dataPack{
          ...
          std::make_tuple(&rotatedFaceDisplacementPtrs,
                          ...
                          Yateto<Config>::Init::rotatedFaceDisplacement::Size),
          std::make_tuple(
              &dofsFaceNodalPtrs, &dofsFaceNodalMemory, Yateto<Config>::Tensor::INodal::size()),
          std::make_tuple(&prevCoefficientsPtrs,
                          &prevCoefficientsMemory,
                          Yateto<Config>::Tensor::nodal::nodes2D::Shape[0])};
      ...
      for (auto& item : dataPack) {
        ...
        memory = reinterpret_cast<RealT*>(
            device.api->getStackMemory(elementSize * numElements * sizeof(RealT)));
      ...
      auto rotateFaceDisplacementKrnl =
          typename Yateto<Config>::Kernel::gpu_rotateFaceDisplacement();
      const auto auxTmpMemSize =
          yateto::getMaxTmpMemRequired(rotateFaceDisplacementKrnl, projectKernelPrototype);
      auto* auxTmpMem =
          reinterpret_cast<RealT*>(device.api->getStackMemory(auxTmpMemSize * numElements));
      ++memCounter;
      ...
      rotateFaceDisplacementKrnl.displacementRotationMatrix =
          const_cast<const RealT**>(rotateDisplacementToFaceNormalPtrs);
      ...
      for (int order = 1; order < Config::ConvergenceOrder + 1; ++order) {
        ...
        auto* derivativesOffsets = timeKernel.getDerivativesOffsets();
        for (unsigned i = 0; i < yateto::numFamilyMembers<typename Yateto<Config>::Tensor::dQ>();
             ++i) {
      ...
      rotateFaceDisplacementKrnl.faceDisplacement =
          const_cast<const RealT**>(rotatedFaceDisplacementPtrs);
      rotateFaceDisplacementKrnl.displacementRotationMatrix =
          const_cast<const RealT**>(rotateDisplacementToGlobalPtrs);
