...
class LayerSet {
  ...
  [[nodiscard]] std::size_t color(typename Definition::Type&& value, RestArgs&&... rest) const {
    return definition.color(std::move(value)) +
           definition.size() * subLayerSet.color(std::forward<RestArgs>(rest)...);
  }
...
class ColorMap {
  ...
  ColorMap(Definitions&&... definitions)
      : layerSets(NestedLayerSets::create(std::forward<Definitions>(definitions)...)) {}
  ...
  [[nodiscard]] std::size_t color(Args&&... args) const {
    return layerSets.color(std::forward<Args>(args)...);
  }
