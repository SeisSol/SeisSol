...
void Spacetime::setGlobalData(const CompoundGlobalData& global) {
  set_kDivMT(krnlPrototype_, global.onHost->stiffnessMatricesTransposed);
  projectDerivativeToNodalBoundaryRotated_.V3mTo2nFace = global.onHost->v3mTo2nFace;

#ifdef ACL_DEVICE
  assert(global.onDevice != nullptr);

  set_kDivMT(deviceKrnlPrototype_, global.onDevice->stiffnessMatricesTransposed);
...
void Spacetime::computeAder(const real* coeffs,
  ...
  if constexpr (Config::GlobalElementwise) {
    setupContainer<tensor::globalMkDivMT>(get_ref_globalMkDivMT(krnl),
                                          data.get<LTS::GlobalMkDivMT>());
  }
