...
#ifdef USE_DAMAGE
void TimeCommon::computeNonIntegrals(
    Time& time,
    LocalData& data,
    ...
    if (faceTypes[dofneighbor] != FaceType::Outflow &&
      ...
      // Then pass it to timeIntegrated[dofneighbor]
        ...
        alignas(PagesizeStack)
            real QInterpolatedBody[ConvergenceOrder][tensor::Q::size()] = {{0.0}};
        real* QInterpolatedBodyi;
        alignas(PagesizeStack)
            real QInterpolatedBodyNodal[ConvergenceOrder][tensor::QNodal::size()];
        ...
        for (unsigned int timeInterval = 0; timeInterval < ConvergenceOrder; ++timeInterval) {
          ...
          time.computeTaylorExpansion(
              timePoints[timeInterval], 0.0, timeDofs[dofneighbor], QInterpolatedBodyi);
        ...
        for (unsigned timeInterval = 0; timeInterval < ConvergenceOrder; ++timeInterval) {
          ...
          for (unsigned q = 0; q < tensor::Q::Shape[0]; ++q) {
            real EspI =
                (exxNodal[q] + epsInitxx) + (eyyNodal[q] + epsInityy) + (ezzNodal[q] + epsInitzz);
