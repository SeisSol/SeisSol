...
void TimeManager::addClusters(const initializer::ClusterLayout& clusterLayout,
  ...
  for (std::size_t localClusterId = 0; localClusterId < clusterLayout.globalClusterCount;
    ...
    // Dynamic rupture
    const auto interiorId =
        initializer::LayerIdentifier(HaloType::Interior, Config(), localClusterId);
    const auto copyId = initializer::LayerIdentifier(HaloType::Copy, Config(), localClusterId);
    const auto ghostId = initializer::LayerIdentifier(HaloType::Ghost, Config(), localClusterId);
    ...
    for (auto type : {HaloType::Copy, HaloType::Interior}) {
      const auto offsetMonitoring =
          type == HaloType::Interior ? 0 : clusterLayout.globalClusterCount;
      ...
      const bool printProgress =
          (localClusterId == clusterLayout.globalClusterCount - 1) && (type == HaloType::Interior);
      const auto profilingId = localClusterId + offsetMonitoring;
      auto* layerData =
          &memoryManager.getLtsTree()->layer(type == HaloType::Copy ? copyId : interiorId);
      ...
      const auto dynRupSize =
          type == HaloType::Copy ? dynRupCopyData->size() : dynRupInteriorData->size();
