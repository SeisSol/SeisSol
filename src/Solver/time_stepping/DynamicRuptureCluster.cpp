...
SCOREP_USER_REGION( "computeDynamicRupture", SCOREP_USER_REGION_TYPE_FUNCTION )
  ...
  if (layer->getNumberOfCells() > 0) {
    ...
    if (frictionSolverDevice->allocationPlace() == initializer::AllocationPlace::Host) {
      layer->varSynchronizeTo(descr->qInterpolatedPlus, initializer::AllocationPlace::Host, streamRuntime.stream());
      layer->varSynchronizeTo(descr->qInterpolatedMinus, initializer::AllocationPlace::Host, streamRuntime.stream());
    }
    ...
    device.api->popLastProfilingMark();
    if (frictionSolverDevice->allocationPlace() == initializer::AllocationPlace::Host) {
      layer->varSynchronizeTo(descr->fluxSolverMinus, initializer::AllocationPlace::Device, streamRuntime.stream());
      layer->varSynchronizeTo(descr->fluxSolverPlus, initializer::AllocationPlace::Device, streamRuntime.stream());
      layer->varSynchronizeTo(descr->imposedStateMinus, initializer::AllocationPlace::Device, streamRuntime.stream());
      layer->varSynchronizeTo(descr->imposedStatePlus, initializer::AllocationPlace::Device, streamRuntime.stream());
    }
  }
  loopStatistics->end(regionComputeDynamicRupture, layer->getNumberOfCells(), profilingId);
#endif
  ...
  for (unsigned face = 0; face < layer->getNumberOfCells(); ++face) {
    ...
    dynamicRuptureKernel.flopsGodunovState(faceInformation[face], faceNonZeroFlops, faceHardwareFlops);
...
void DynamicRuptureCluster::runCompute(ComputeStep step) {
        ...
        // maybe output
        if (computePickpoint) {
            faultOutputManager->writePickpointOutput(ct.time[ComputeStep::Interact] + timeStepSize(), timeStepSize(), streamRuntime);
        }
    }

void DynamicRuptureCluster::synchronizeTo(seissol::initializer::AllocationPlace place, void* stream) {
#ifdef ACL_DEVICE
