...
void ReceiverWriter::addPoints(const seissol::geometry::MeshReader& mesh,
  ...
  for (unsigned point = 0; point < numberOfPoints; ++point) {
    if (contained[point] == 1) {
      ...
      const auto layer = position.first;
      ...
      m_receiverClusters[layer].addReceiver(meshId, point, points[point], mesh, container);
    }
  }
}
...
kernels::ReceiverCluster*
    ReceiverWriter::receiverCluster(const initializer::LayerIdentifier& identifier) {
  assert(identifier.halo != HaloType::Ghost);
  const auto clusterId = colorMap.value().colorId(identifier);
