...
std::string OpaqueDatatype::toStringRaw(const void* data) const {
  ...
  for (std::size_t i = 0; i < (sizeP / 3) * 3; i += 3) {
  ...
  if (sizeP % 3 == 2) {
    base64Convert<3>(sstr, dataPtr[sizeP - 2], dataPtr[sizeP - 1], 0);
    sstr << Base64Pad;
  }
...
std::optional<std::vector<uint8_t>> OpaqueDatatype::fromStringRaw(const std::string& str) const {
  std::vector<uint8_t> data(sizeP);
  for (std::size_t i = 0; i < sizeP / 3; ++i) {
    base64Revert<3>(str, 4 * i, data.data(), 3 * i);
  }
  const auto rest = (str.size() / 4) * 4;
  if (sizeP % 3 == 1) {
    base64Revert<1>(str, rest, data.data(), (sizeP / 3) * 3);
  }
  if (sizeP % 3 == 2) {
    base64Revert<2>(str, rest, data.data(), (sizeP / 3) * 3);
  }
  return std::make_optional(data);
}
