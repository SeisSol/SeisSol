...
void fitAttenuation(seissol::model::ViscoElasticMaterial& vm, double freqCentral, double freqRatio) {
#if NUMBER_OF_RELAXATION_MECHANISMS > 0
    constexpr std::size_t nummech = NUMBER_OF_RELAXATION_MECHANISMS;
    constexpr std::size_t kmax = 2 * nummech - 1; // slight note: if nummech == 0, this does not make any sense
...
#error "The attenuation still needs to be ported to this case (anisotropy+anelasticity). It could not be done at the time of writing the code because a respective model was not yet established within SeisSol."
#endif
