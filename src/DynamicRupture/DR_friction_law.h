        ...
        namespace fr_law {
            ...
            public:
                virtual void evaluate(seissol::initializers::Layer&  layerData,
                                       ...
                                       double DeltaT[CONVERGENCE_ORDER]) = 0;
            };
            ...
            public:
                ...
                virtual void evaluate(seissol::initializers::Layer&  layerData,
                        ...
                        double fullUpdateTime,
                        double timeWeights[CONVERGENCE_ORDER],
                        double DeltaT[CONVERGENCE_ORDER]) override {
                    ...
                    double TractionGP_XY[numOfPointsPadded][CONVERGENCE_ORDER] = {{}}; // OUT: updated Traction 2D array with size [1:i_numberOfPoints, CONVERGENCE_ORDER]
                    double TractionGP_XZ[numOfPointsPadded][CONVERGENCE_ORDER] = {{}};// OUT: updated Traction 2D array with size [1:i_numberOfPoints, CONVERGENCE_ORDER]
                    double NorStressGP[numOfPointsPadded][CONVERGENCE_ORDER] = {{}};
                    double XYStressGP[numOfPointsPadded][CONVERGENCE_ORDER]= {{}};
                    double XZStressGP[numOfPointsPadded][CONVERGENCE_ORDER]= {{}};
                    ...
                    assert(QInterpolatedPlus[0][0] ==  QInterpolatedPlus[0][0] );
                    ...
                    double sum_tmpSlip;
                    double tmpSlip[numOfPointsPadded];
                    double matmul[numOfPointsPadded];
                    ...
                    double tn = fullUpdateTime;
                    double time_inc = 0;
                    double P[numOfPointsPadded];
                    double Strength[numOfPointsPadded];
                    double ShTest[numOfPointsPadded];
                    double LocSR[numOfPointsPadded];
                    double LocSR1[numOfPointsPadded];
                    double LocSR2[numOfPointsPadded];
                    double LocTracXY[numOfPointsPadded];
                    double LocTracXZ[numOfPointsPadded];
                    double f1;
                    double f2;
                    double test3[numOfPointsPadded];
                    double test5[numOfPointsPadded];
                    ...
                    for (int iTimeGP = 0; iTimeGP < CONVERGENCE_ORDER; iTimeGP++) {  //loop over time steps
                        ...
                        for(int iBndGP = 0; iBndGP < numOfPointsPadded; iBndGP++) {
                            ...
                            //TODO: check access of initialStressInFaultCS
                            ...
                            ShTest[iBndGP] = std::sqrt(
                                    std::pow( initialStressInFaultCS[face][iBndGP][3]+ XYStressGP[iBndGP][iTimeGP], 2) +
                                    std::pow(initialStressInFaultCS[face][iBndGP][5] + XZStressGP[iBndGP][iTimeGP], 2));
                            ...
                            */
                            ...
                            LocSR1[iBndGP] =  LocSR[iBndGP] * (initialStressInFaultCS[face][iBndGP][3] + XYStressGP[iBndGP][iTimeGP]) /
                                              (std::max(ShTest[iBndGP] , Strength[iBndGP]) );
                            LocSR2[iBndGP] =  LocSR[iBndGP] * (initialStressInFaultCS[face][iBndGP][5] + XZStressGP[iBndGP][iTimeGP]) /
                                              (std::max(ShTest[iBndGP] , Strength[iBndGP]) );
                        ...
                        for(int iBndGP = 0; iBndGP < numberOfPoints; iBndGP++) {
                            ...
                            mu[face][iBndGP]= mu_S[face][iBndGP]- (mu_S[face][iBndGP] - mu_D[face][iBndGP]) * std::max(f1, f2);
