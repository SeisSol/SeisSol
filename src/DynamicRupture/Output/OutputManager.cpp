...
#pragma omp parallel for schedule(static)
    ...
    seissolInstance_.faultWriter().init(cellConnectivity.data(),
                                        ...
                                        seissolInstance_.outputPrefix().data(),
                                        printTime,
                                        backendType,
                                        backupTimeStamp_);
...
void OutputManager::initPickpointOutput() {
  ...
  for (auto& [id, outputData] : ppOutputData_) {
    ...
    if (allReceiversInOneFilePerRank) {
      ...
      files.resize(1);
      auto fileName = buildIndexedMPIFileName(seissolInstance_.outputPrefix(), -1, "faultreceiver");
      fileName += ".dat";
      ...
      for (const auto& [index, receivers] : globalIndexMap) {
        auto fileName =
            buildIndexedMPIFileName(seissolInstance_.outputPrefix(), index + 1, "faultreceiver");
