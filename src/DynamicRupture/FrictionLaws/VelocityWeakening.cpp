    ...
    */
    ...
    for (int iBndGP = 0; iBndGP < numberOfPoints; iBndGP++) {
      ...
      LocSlip = slip[ltsFace][iBndGP]; // DISC%DynRup%Slip(iBndGP,iFace)              //!< Slip path
                                       // at given fault node
      LocSlip1 = slipStrike[ltsFace][iBndGP]; // DISC%DynRup%Slip1(iBndGP,iFace)            //!<
                                              // Slip at given fault node along loc dir 1
      LocSlip2 = slipDip[ltsFace][iBndGP]; // DISC%DynRup%Slip2(iBndGP,iFace)            // !< Slip
                                           // at given fault node along loc dir 2
      LocSR1 = slipRateStrike[ltsFace][iBndGP]; // DISC%DynRup%SlipRate1(iBndGP,iFace)         // !<
                                                // Slip Rate at given fault node
      LocSR2 = slipRateDip[ltsFace][iBndGP];    // DISC%DynRup%SlipRate2(iBndGP,iFace)         // !<
                                                // Slip Rate at given fault node
      ...
      for (int iTimeGP = 0; iTimeGP < CONVERGENCE_ORDER; iTimeGP++) {
        ...
        P = LocP + P_0;
        ShTest = std::sqrt(seissol::dr::aux::power(initialStressInFaultCS[ltsFace][iBndGP][3] +
        ...
        SlipRateMagnitude[ltsFace][iBndGP] =
            std::sqrt(seissol::dr::aux::power(LocSR1, 2) +
                      seissol::dr::aux::power(LocSR2, 2)); // can be put outside of the loop
        ...
        for (int j = 0; j < nSVupdates; j++) { //! This loop corrects SV values
          ...
          //!
          SRtest = SlipRateMagnitude[ltsFace][iBndGP]; // We use as first guess the SR value of the
                                                       // previous time step
          for (int i = 0; i < nSRupdates; i++) {       //! This loop corrects SR values
            tmp = m_Params->rs_f0 + RS_a[ltsFace] * SRtest / (SRtest + m_Params->rs_sr0) -
                  m_Params->rs_b * LocSV / (LocSV + RS_sl0[ltsFace]); //=mu
            NR = -impAndEta[ltsFace].inv_eta_s * (abs(P) * tmp - ShTest) - SRtest;
            dNR = -impAndEta[ltsFace].inv_eta_s *
                      (abs(P) * (RS_a[ltsFace] / (SRtest + m_Params->rs_sr0) -
                                 RS_a[ltsFace] * SRtest /
                                     seissol::dr::aux::power(SRtest + m_Params->rs_sr0, 2))) -
                  1.0;
            SRtest = SRtest - NR / dNR;
          } // End nSRupdates-Loop
          tmp = 0.5 * (SlipRateMagnitude[ltsFace][iBndGP] +
                       abs(SRtest)); // For the next SV update, use the mean slip rate between the
        ...
        tmp = 0.5 * (SlipRateMagnitude[ltsFace][iBndGP]) / m_Params->rs_sr0 *
              exp((m_Params->rs_f0 +
                   m_Params->rs_b * log(m_Params->rs_sr0 * LocSV / RS_sl0[ltsFace])) /
                  RS_a[ltsFace]);
        ...
        LocMu = m_Params->rs_f0 +
                RS_a[ltsFace] * SlipRateMagnitude[ltsFace][iBndGP] /
                    (SlipRateMagnitude[ltsFace][iBndGP] + m_Params->rs_sr0) -
                m_Params->rs_b * LocSV / (LocSV + RS_sl0[ltsFace]);
        ...
        // by a free surface!)
        LocSR1 =
            -impAndEta[ltsFace].inv_eta_s * (LocTracXY - faultStresses.XYStressGP[iTimeGP][iBndGP]);
        LocSR2 =
            -impAndEta[ltsFace].inv_eta_s * (LocTracXZ - faultStresses.XZStressGP[iTimeGP][iBndGP]);
