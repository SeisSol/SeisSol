    ...
    for (unsigned ltsFace = 0; ltsFace < layerData.getNumberOfCells(); ++ltsFace) {
      ...
      for (unsigned o = 0; o < CONVERGENCE_ORDER; ++o) {
        for (unsigned i = 0; i < seissol::dr::misc::numPaddedPoints;
          ...
          qStressIPlus[o][XX][i] = (lambda0P*EspIp)
                + (2*(mu0P)
                  )
                  *(qIPlus[o][XX][i]+epsInitxx);

          qStressIPlus[o][YY][i] = (lambda0P*EspIp)
                + (2*(mu0P)
                  )
                  *(qIPlus[o][YY][i]+epsInityy);

          qStressIPlus[o][ZZ][i] = (lambda0P*EspIp)
                + (2*(mu0P)
                  )
          ...
          qStressIMinus[o][XX][i] = (lambda0M*EspIm)
                + (2*(mu0M)
                  )
                  *(qIMinus[o][XX][i]+epsInitxx);

          qStressIMinus[o][YY][i] = (lambda0M*EspIm)
                + (2*(mu0M)
                  )
                  *(qIMinus[o][YY][i]+epsInityy);

          qStressIMinus[o][ZZ][i] = (lambda0M*EspIm)
                + (2*(mu0M)
                  )
