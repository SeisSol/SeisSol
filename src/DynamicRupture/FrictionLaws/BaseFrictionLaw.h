...
class BaseFrictionLaw : public FrictionSolver {
  ...
  void copyData(seissol::initializers::Layer& layerData,
  ...
  void evaluateSingle(unsigned ltsFace, const TimeWeightType timeWeights) override {
  ...
  void evaluate(seissol::initializers::Layer& layerData,
    ...
    copyData(layerData, dynRup, fullUpdateTime);
...
#pragma omp parallel for schedule(static)
    ...
    for (unsigned ltsFace = 0; ltsFace < layerData.getNumberOfCells(); ++ltsFace) {
      evaluateSingle(ltsFace, timeWeights);
    }
  }
};
} // namespace seissol::dr::friction_law
