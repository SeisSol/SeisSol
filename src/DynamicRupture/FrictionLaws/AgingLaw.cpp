...
//      real localSlipRateStrike = slipRateStrike[ltsFace][pointIndex]; // Slip rate along direction
//      1 real localSlipRateDip = slipRateDip[ltsFace][pointIndex];       // Slip rate along
//      direction 2 real localStateVariable = StateVariable[ltsFace][pointIndex];   // State
//      Variable real initialPressure = initialStressInFaultCS[ltsFace][pointIndex][0]; // initial
//      pressure
//
//      real localMu = 0;
...
//          localStateVariable =
//              calcStateVariableHook(stateVariable, tmp, timeIncrement,
...
//          // which has
//          //  a derivative dNR = d(NR)/d(SR). We can then find SR by iterating SR_{i+1}=SR_i-(
//          NR_i
//          //  / dNR_i ).
...
//        // LocTrac  = ABS(LocTrac)*(-SignSR)  !!! line commented as it leads NOT to correct
//        results
...
//
//        // Update slip rate (notice that LocSR(T=0)=-2c_s/mu*s_xy^{Godunov} is the slip rate
//        caused
//        // by a free surface!)
