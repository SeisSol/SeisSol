...
class RateAndStateBase : public BaseFrictionSolver<RateAndStateBase<Derived, TPMethod>> {
  ...
  void preHook(real (*stateVariableBuffer)[misc::numPaddedPoints]) {
    ...
    #pragma omp target teams distribute depend(inout: *queue) device(TARGETDART_ANY) map(to: devLocalStateVariable[0:layerSize]) map(from: stateVariableBuffer[0:layerSize]) nowait
