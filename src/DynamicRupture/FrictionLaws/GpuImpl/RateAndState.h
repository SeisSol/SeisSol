...
class RateAndStateBase : public BaseFrictionSolver<RateAndStateBase<Derived, TPMethod>> {
  ...
  void preHook(real (*stateVariableBuffer)[misc::numPaddedPoints]) {
    ...
    #pragma omp target teams distribute map(to: devLocalStateVariable[0:layerSize]) map(from: stateVariableBuffer[0:layerSize]) nowait
  ...
  void updateStateVariableIterative(unsigned timeIndex) {
    ...
    for (unsigned j = 0; j < this->settings.numberStateVariableUpdates; j++) {
      ...
      // #pragma omp distribute
      #pragma omp target teams distribute map(to: devStateVariableBuffer[0:layerSize], devNormalStress[0:layerSize], devAbsoluteShearStress[0:layerSize], devImpAndEta[0:layerSize]) map(tofrom: devSlipRateMagnitude[0:layerSize]) map(from: devHasConverged[0:layerSize], devLocalSlipRate[0:layerSize], devMu[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
  ...
  void saveDynamicStressOutput() {
    ...
    // #pragma omp distribute
    #pragma omp target teams distribute map(to: devMu[0:layerSize], devRuptureTime[0:layerSize]) map(tofrom: devDynStressTimePending[0:layerSize]) map(from: devDynStressTime[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for schedule(static, 1)
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
  ...
  void updateNormalStress(size_t timeIndex) {
    ...
    // #pragma omp distribute
    #pragma omp target teams distribute map(to: devFaultStresses[0:layerSize], devInitialStressInFaultCS[0:layerSize], tpCurrentLayerDetails) map(from: devNormalStress[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for schedule(static, 1)
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
        auto& faultStresses = devFaultStresses[ltsFace];
