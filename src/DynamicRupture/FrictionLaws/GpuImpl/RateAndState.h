...
class RateAndStateBase : public BaseFrictionSolver<RateAndStateBase<Derived, TPMethod>> {
  ...
  void copySpecificLtsDataTreeToLocal(seissol::initializer::Layer& layerData,
    ...
    auto* concreteLts = dynamic_cast<seissol::initializer::LTSRateAndState const* const>(dynRup);
    this->a = layerData.var(concreteLts->rsA, seissol::initializer::AllocationPlace::Host);
    this->sl0 = layerData.var(concreteLts->rsSl0, seissol::initializer::AllocationPlace::Host);
    this->stateVariable =
        layerData.var(concreteLts->stateVariable, seissol::initializer::AllocationPlace::Host);
  ...
  void preHook(real (*stateVariableBuffer)[misc::numPaddedPoints]) {
    ...
    #pragma omp target teams distribute depend(inout: queue[0]) device(TARGETDART_ANY) map(to: devLocalStateVariable[0:layerSize]) map(from: stateVariableBuffer[0:layerSize]) nowait
  ...
  void saveDynamicStressOutput() {
    ...
    // #pragma omp distribute
    #pragma omp target teams distribute depend(inout: queue[0]) device(TARGETDART_ANY) map(to: devMu[0:layerSize], devRuptureTime[0:layerSize]) map(tofrom: devDynStressTimePending[0:layerSize]) map(from: devDynStressTime[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for 
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
  ...
  void updateNormalStress(size_t timeIndex) {
    ...
    // #pragma omp distribute
    #pragma omp target teams distribute depend(inout: queue[0]) device(TARGETDART_ANY) map(to: devFaultStresses[0:layerSize], devInitialStressInFaultCS[0:layerSize]) map(from: devNormalStress[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for 
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
