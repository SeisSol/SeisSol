...
constexpr std::size_t PaddedMultiple = safeblockMultiple(seissol::dr::misc::NumPaddedPoints, BlockTargetsize);

template<typename T>
__launch_bounds__(PaddedMultiple * seissol::dr::misc::NumPaddedPoints)
__global__ void flkernelwrapper (int elements, FrictionLawData* data, double* timeWeights, real* spaceWeights, real* resampleMatrix, real fullUpdateTime,
...
template<typename T>
void BaseFrictionSolver<T>::evaluateKernel(seissol::parallel::runtime::StreamRuntime& runtime, real fullUpdateTime) {
#ifdef __CUDACC__
    ...
    flkernelwrapper<T><<<grid, block, 0, stream>>>(this->currLayerSize, data, devTimeWeights, devSpaceWeights, resampleMatrix, fullUpdateTime, devTpInverseFourierCoefficients, devTpGridPoints, devHeatSource);
}

template class BaseFrictionSolver<NoFault>;
template class BaseFrictionSolver<LinearSlipWeakeningBase<LinearSlipWeakeningLaw<NoSpecialization>>>;
...
template class BaseFrictionSolver<RateAndStateBase<SlowVelocityWeakeningLaw<AgingLaw<NoTP>, NoTP>, NoTP>>;
template class BaseFrictionSolver<RateAndStateBase<SlowVelocityWeakeningLaw<SlipLaw<NoTP>, NoTP>, NoTP>>;
template class BaseFrictionSolver<RateAndStateBase<FastVelocityWeakeningLaw<NoTP>, NoTP>>;
template class BaseFrictionSolver<RateAndStateBase<SlowVelocityWeakeningLaw<AgingLaw<ThermalPressurization>, ThermalPressurization>, ThermalPressurization>>;
template class BaseFrictionSolver<RateAndStateBase<SlowVelocityWeakeningLaw<SlipLaw<ThermalPressurization>, ThermalPressurization>, ThermalPressurization>>;
template class BaseFrictionSolver<RateAndStateBase<FastVelocityWeakeningLaw<ThermalPressurization>, ThermalPressurization>>;
