...
class LinearSlipWeakeningBase : public BaseFrictionSolver<LinearSlipWeakeningBase<Derived>> {
  ...
  void calcSlipRateAndTraction(FaultStresses* devFaultStresses,
    ...
    #pragma omp target teams distribute map(to: devInitialStressInFaultCS[0:layerSize], devImpAndEta[0:layerSize]) map(tofrom: devSlipRateMagnitude[0:layerSize], devSlipRate1[0:layerSize], devSlipRate2[0:layerSize], devSlip1[0:layerSize], devSlip2[0:layerSize]) map(from: devTraction1[0:layerSize], devTraction2[0:layerSize]) nowait
  ...
  void frictionFunctionHook(real (*stateVariableBuffer)[misc::numPaddedPoints]) {
    ...
    // #pragma omp distribute
    #pragma omp target teams distribute map(to: devMuS[0:layerSize], devMuD[0:layerSize], stateVariableBuffer[0:layerSize]) map(from: devMu[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for schedule(static, 1)
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
  ...
  void saveDynamicStressOutput() {
    ...
    // #pragma omp distribute
    #pragma omp target teams distribute map(to: devAccumulatedSlipMagnitude[0:layerSize], devDC[0:layerSize]) map(tofrom: devDynStressTimePending[0:layerSize]) map(from: devDynStressTime[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for schedule(static, 1)
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
...
class LinearSlipWeakeningLaw
  ...
  void calcStrengthHook(FaultStresses* devFaultStresses,
    ...
    // #pragma omp distribute
    #pragma omp target teams distribute map(to: currentLayerDetails, devMu[0:layerSize], devCohesion[0:layerSize], devSlipRateMagnitude[0:layerSize], devInitialStressInFaultCS[0:layerSize], devFaultStresses[0:layerSize]) map(from: devStrengthBuffer[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for schedule(static, 1)
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
  ...
  void calcStateVariableHook(real (*devStateVariableBuffer)[misc::numPaddedPoints],
    ...
    // #pragma omp distribute
    #pragma omp target teams distribute map(to: devSlipRateMagnitude[0:layerSize], devForcedRuptureTime[0:layerSize], devDC[0:layerSize], devResample[0:layerSize]) map(tofrom: devAccumulatedSlipMagnitude[0:layerSize]) map(from: devStateVariableBuffer[0:layerSize]) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for schedule(static, 1)
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
