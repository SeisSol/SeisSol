...
class FastVelocityWeakeningLaw
  ...
  void updateStateVariable(double timeIncrement) {
    ...
    #pragma omp target distribute map(to: details, devStateVarReference[0:layerSize], devLocalSlipRate[0:layerSize]) map(from: devStateVariableBuffer[0:layerSize]) nowait
  ...
  void resampleStateVar(real (*devStateVariableBuffer)[misc::numPaddedPoints]) {
    ...
    #pragma omp target distribute map(to: devStateVariableBuffer[0:layerSize], resampleMatrix) map(tofrom: devStateVariable[0:layerSize]) allocate(omp_pteam_mem_alloc:deltaStateVar) nowait
      for (int ltsFace = 0; ltsFace < layerSize; ++ltsFace) {
        #pragma omp parallel for schedule(static, 1)
        for (int pointIndex = 0; pointIndex < misc::numPaddedPoints; ++pointIndex) {
