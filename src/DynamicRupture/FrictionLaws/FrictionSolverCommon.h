...
#pragma omp simd
    ...
    for (size_t index = Range::Start; index < Range::End; index += Range::Step) {
      ...
      // instead of computing the slip rate magnitude from the differences in velocities calculated
      // above (magnitude of the vector (slipRateMagnitudei)). The moment magnitude based on
      // (slipRateMagnitudei) is typically non zero at the end of the earthquake (probably because
      // it incorporates the velocity discontinuities inherent of DG methods, including the
      // contributions of fault normal velocity discontinuity)
